...
use zodiac_parsing::abstract_syntax::{AbstractSyntaxTokenizer, AbstractSyntaxNode, AbstractSyntaxTokenError};
...
fn parse_rect_produces_rectangle_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Rectangle, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
...
fn parse_circle_produces_circle_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Circle, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn parse_text_produces_text_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Text, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn parse_stroke_width_produces_stroke_width_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Rectangle, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::StrokeWidth(1), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn parse_radius_produces_radius_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Circle, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::Radius(1), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn glyph_index_produces_glyph_index_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Text, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::GlyphIndex(1), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn position_produces_glyph_position_node() {
    ...
    assert_eq!(AbstractSyntaxNode::Text, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::Position((100, 200)), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
...
fn parse_with_multiple_properties_produces_correct_nodes() {
    ...
    assert_eq!(AbstractSyntaxNode::Circle, tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::Radius(1), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::StrokeWidth(2), tokenizer.next().unwrap().unwrap());
    assert_eq!(AbstractSyntaxNode::CompleteControl, tokenizer.next().unwrap().unwrap());
    assert_eq!(None, tokenizer.next());
}
